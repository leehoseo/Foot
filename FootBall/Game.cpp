...
void Game::Render()
{
	...
	Screen::Instance()->ScreenClear();

	RenderScene();
	
	// Note: 렌더링 끝 
	Screen::Instance()->ScreenFlipping();
}
...
void Game::ChangeStage(Stage * _newStage)
{
	SAFE_DELETE(m_pStage);
	m_pStage = _newStage;
}

void Game::CollisionCheckBall_GP()
{
...
void Game::RenderScene()
{
	...
	case GAME_STATE::INIT:
		//if (g_nStage == 0)
		Screen::Instance()->InitScreen();
		break;
	case GAME_STATE::READY:
		Screen::Instance()->ReadyScreen( m_nCurStage );
		break;
	case GAME_STATE::RUNNING:
		Screen::Instance()->BackScreen( *m_pStage, m_nCurStage, m_nBallCount, m_GameStartTime );
		m_pGoalPost->Render();

		// Note: 효과 출력  	
		if ( m_IsGoal == true )
		{
			Screen::Instance()->GoalMessage( 10, 10 );
		}
	...
	case GAME_STATE::SUCCESS:
		Screen::Instance()->SuccessScreen();
		break;
	case GAME_STATE::FAILED:
		Screen::Instance()->FailureScreen();
		break;
	case GAME_STATE::RESULT:
		Screen::Instance()->ResultScreen( m_nCurStage );
		break;
	}
}
