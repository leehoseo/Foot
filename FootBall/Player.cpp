...
void Player::Render()
{
	...
	if (m_nX < 2)  //  왼쪽 클리핑 처리  g_nLength : Player
		Screen::Instance()->ScreenPrint(2, m_nMoveY, &m_strPlayer[(m_nX - 2)*-1]);	 // 좌표를 배열 인덱스 
	else if (m_nMoveX + (m_nLength - m_nCenterX + 1) > 43) // 오른쪽 클리핑 처리
	{
		strncat_s(string, m_strPlayer, m_nLength - ((m_nMoveX + m_nCenterX + 1) - 43));
		Screen::Instance()->ScreenPrint(m_nX, m_nMoveY, string);
	}
	else { // 1 컬럼씩 이동
		Screen::Instance()->ScreenPrint(m_nX, m_nMoveY , m_strPlayer);
	}
}
